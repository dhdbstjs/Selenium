...
driver = webdriver.Chrome(service=Service(ChromeDriverManager(driver_version=latest_version).install()))
...
if len(console_error) >= 1:
    for a in range(len(console_error)):
        print('CONSOLE_ERROR : ' + console_error[a]['level'] + ' >>> ' + console_error[a]['message'] + '\n'
